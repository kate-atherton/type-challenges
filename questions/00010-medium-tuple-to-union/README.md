...
- TupleToUnion converts tuple type to union type
- We provide TupleToUnion type T, which must be an array (since we are using extends)
- We use a type conditional expression which will always return true, as this allows us to infer the array type which we are calling U
- We infer the type of array with `Array<infer U>`, this will give us type a | type b etc. based on the types in the tuple, and in turn returns a union type by default
- U (the union type) is always returned since only arrays are accepted as arguments
