...
- Takes an object and returns an object where all properties and nested properties are readonly
- T refers to the object we are passing in
- `P in keyof T` goes through every key in the object
- Conditional statement `keyof T[P] extends never` checks if the key value is an object with more keys.
- If the key value type is a primitive value eg. 'foo: string', then we return `readonly foo: string`
- If the key value type is another object, we recurse to return all the readonly values of any number of nested objects.
